  ...
  for (size_t iteration = 0;; ++iteration)
  {
    ...
    while (!open.empty())
    {
      ...
      if (!getEarliestConflict(P.solution, robots, col_mng_robots, robot_objs, inter_robot_conflict))
      {
        ...
        // get the discrete search solution
        ...
        bool feasible = execute_optimizationMetaRobot(problem, // inputFile
        ...
        if (feasible)
        {
          ...
          duration total_runtime = now - dbcbs_start;
          duration_opt = now - opt_start;
          float cost = optimization_sol.get_cost();
          stats << "  - t: " << total_runtime.count() << "\n";
          stats << "    cost: " << cost << "\n";
