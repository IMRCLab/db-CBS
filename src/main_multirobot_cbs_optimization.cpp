...
int main(int argc, char *argv[]) {
  ...
  int opt_id = 1;
  // when each Node has a cluster ONLY which will be optimized, and checked, divided into clusters again after the optimization.
  if(only_max){ // take MAX conflict node
    ...
    while(!open_opt.empty()){
      ...
      // optimize jointly the cluster (N.cluster) that this Node has, since the collision between this pair is MAX
      ...
      if(feasible){
        ...
        N.conflict = getConflicts(N.multirobot_trajectory.trajectories, robots, col_mng_robots, robot_objs, N.conflict_matrix);
        if(N.conflict == 0){
          std::cout << "No inter-robot conflict" << std::endl;
          N.multirobot_trajectory.to_yaml_format(outFile.c_str());
          return 0;
  ...
  // when each cluster inside the loop is optimized and Node.conflict is equal to the maximum number of the collision happening here
  // take MIN conflict node
  else {
        ...
        for (size_t j = 0; j <= i; j++){
          if(N.conflict_matrix[i][j] > 0){
            ...
            if(feasible){
              ...
              if(newNode.conflict < 1){
                std::cout << "No inter-robot conflict" << std::endl;
                newNode.multirobot_trajectory.to_yaml_format(outFile.c_str());
                return 0;
              }

              std::cout << "new Node conflict (optimized): " << newNode.conflict << std::endl;
              auto handle = open_opt.push(newNode);
              (*handle).handle = handle;
            }
            ++opt_id;
